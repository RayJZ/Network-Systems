...
void handle_request(int client_socket_descriptor, char* request)
{
    ...
    fprintf(stderr,"handling command: %s, filename: %s\n", command,filename);
    //handle command
    if(strcmp(command,"put")==0)
    {
        int chunk_size = atoi(strtok(NULL, " "));
        fprintf(stderr, "putting\n");
        memset(buf,0,sizeof(buf));

        long bytes_read = strlen(request);

        FILE * file = fopen(path,"wb+");
        int i=0;
        while(bytes_read < (chunk_size+strlen(request)) && i++<10)
        { //you are really close to figuring this out. the issue is that the first read sometimes has sections of the
            //fprintf(stderr, "abab: buf: %s\n",buf);
            //memset(buf,0,sizeof(buf));
            ssize_t read_success = read(client_socket_descriptor,buf,MAXCHAR);
            if(read_success<0)
            {
            ...
            fwrite(buf,1,read_success,file);
            fprintf(stderr, "bytes_read: %ld/%lu, read_success: %zd\n",bytes_read,chunk_size+strlen(request),read_success);
        }
        fprintf(stderr,"\n");
        fclose(file);
    }
    else if(strcmp(command,"get")==0)
    {
        ...
        if(file)
        {
            size_t amt = fread(buf,1,MAXCHAR,file);
            fprintf(stderr,"writing %zu bytes to client",amt);
            write(client_socket_descriptor,buf,amt);
            fprintf(stderr,"\n");
        }
        fclose(file);
    }
    ...
    {
        char ls_command[strlen("ls -l  | grep ") + strlen(filename) + strlen(cache_directory)];
        if(strcmp(filename,"NULL")==0)
            sprintf(ls_command, "ls -l %s", cache_directory);
        else
            sprintf(ls_command, "ls -l %s | grep %s", cache_directory, filename);
        fprintf(stderr,"ls_command: %s",ls_command);
        FILE * file = popen(ls_command, "r");
        memset(buf,0,MAXCHAR);
        fread(buf,MAXCHAR,1,file);
        pclose(file);
        fprintf(stderr,"ls output: %s\n",buf);

        write(client_socket_descriptor,buf, strlen(buf));
    }
